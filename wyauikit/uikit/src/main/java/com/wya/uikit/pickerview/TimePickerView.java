...
public class TimePickerView extends LinearLayout {
	...
	private void setWheelViewAdapter() {
		...
		yearWheel.setAdapter(new TimePickerAdapter(mStartYear, mEndYear, 1));
		...
		monthListener = new OnItemSelectedListener() {
			...
			public void onItemSelected(int index) {
				...
				dayWheel.setAdapter(new TimePickerAdapter(daystarts, dayends, 1));
	...
	private void setHourMinSecAdapter() {
		hourWheel.setAdapter(new TimePickerAdapter(0, 23, hourSpace));
		hourIndex = selectDate.get(Calendar.HOUR_OF_DAY) / hourSpace;
		hourWheel.setCurrentItem(hourIndex);

		minWheel.setAdapter(new TimePickerAdapter(0, 59, minuteSpace));
		minIndex = selectDate.get(Calendar.MINUTE) / minuteSpace;
		minWheel.setCurrentItem(minIndex);

		secWheel.setAdapter(new TimePickerAdapter(0, 59, secondSpace));
	...
	public int getYear() {
		return type[0] ? (int) yearWheel.getAdapter().getItem(yearIndex) : 0;
	}

	public int getMonth() {
		return type[1] ? (int) monthWheel.getAdapter().getItem(monthIndex) : 0;
	}

	public int getDay() {
		return type[2] ? (int) dayWheel.getAdapter().getItem(dayIndex) : 0;
	}

	public int getHour() {
		return type[3] ? (int) hourWheel.getAdapter().getItem(hourIndex) : 0;
	}

	public int getMinute() {
		return type[4] ? (int) minWheel.getAdapter().getItem(minIndex) : 0;
	}

	public int getSecond() {
		return type[5] ? (int) secWheel.getAdapter().getItem(secIndex) : 0;
	}
