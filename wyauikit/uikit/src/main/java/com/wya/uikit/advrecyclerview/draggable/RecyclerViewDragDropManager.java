...
package com.wya.uikit.advrecyclerview.draggable;
...
import com.wya.uikit.advrecyclerview.adapter.AdapterPath;
import com.wya.uikit.advrecyclerview.adapter.ItemIdComposer;
import com.wya.uikit.advrecyclerview.utils.CustomRecyclerViewUtils;
...
public class RecyclerViewDragDropManager implements DraggableItemConstants {
    ...
    private boolean checkConditionAndStartDragging(RecyclerView rv, MotionEvent e, boolean checkTouchSlop) {
        ...
        final int wrappedItemPosition = WrapperAdapterUtils.INSTANCE.unwrapPosition(rootAdapter, mWrapperAdapter, null, holder.getAdapterPosition(), path);
    ...
    private boolean canStartDrag(RecyclerView.ViewHolder holder, int touchX, int touchY) {
        ...
        final int wrappedItemPosition = WrapperAdapterUtils.INSTANCE.unwrapPosition(mRecyclerView.getAdapter(), mWrapperAdapter, null, origRootPosition);
    ...
    private int getWrappedAdapterPosition(RecyclerView.ViewHolder vh) {
        ...
        return WrapperAdapterUtils.INSTANCE.unwrapPosition(mRecyclerView.getAdapter(), mWrapperAdapter, mComposedAdapterTag, vh.getAdapterPosition());
    }
    
    // TODO: 2019-05-15 ZCQ TEST
    private ItemDraggableRange convertToRootAdapterRange(AdapterPath path, ItemDraggableRange src) {
        final RecyclerView.Adapter rootAdapter = mRecyclerView.getAdapter();
        
        final int start = WrapperAdapterUtils.INSTANCE.wrapPosition(path, mWrapperAdapter, rootAdapter, src.getStart());
        final int end = WrapperAdapterUtils.INSTANCE.wrapPosition(path, mWrapperAdapter, rootAdapter, src.getEnd());
