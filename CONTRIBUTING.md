...
这篇指南会指导你如何为 `wya-android-kit` 贡献一份自己的力量，请在你要提 `issue` 或者 `pull request` 之前花几分钟来阅读一遍这篇指南。
...
基于我们的 [发布周期](./CHANGELOG.md)，我们每个月都会从 `master` 分支切一个 `develope` 分支出来（比如 `develope-3.1` 分支用来发布 3.1 版本）。 如果你要修一个 `bug`，那么请发 `pull request` 到 `master`；如果你要提一个增加新功能的 `pull request`，那么请基于 `develope` 分支来做。

## Bugs

我们使用 [`GitHub Issues`](https://github.com/wya-team/wya-android-kit/issues) 来做 `bug` 追踪。

在你报告一个 `bug` 之前，请先确保已经搜索过已有的 `issue` 和阅读了我们的 [常见问题](https://github.com/wya-team/wya-android-kit/wiki/FAQ)。
